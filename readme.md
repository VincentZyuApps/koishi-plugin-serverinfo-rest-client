...
| --- | --- | --- | --- |
| `按钮菜单` | `button-menu` | `mcinfo1.按钮菜单 [页码]` | QQ 平台两页按钮菜单，页码默认为 1 |
...
`按钮菜单 [页码]` 仅支持 `qq` 平台，并由 `qqKeyboardEnabled` 独立控制；即使 `qqMarkdownEnabled` 已关闭，用户仍可主动打开按钮菜单。菜单 Markdown 会显示 `serverLabel`、客户端版本、主指令、功能指令前缀和当前页码。

- 第 1 页使用两列四行展示八个服务器与概览入口。
- 第 2 页使用两列三行展示六个玩家与账号入口；添加白名单、查询白名单绑定、移除白名单和执行命令等管理员指令不会进入菜单。
- 两页底部始终显示上一页和下一页；边界按钮带有 `❌`，点击后分别提示已经是第一页或最后一页。
- 绑定玩家和玩家在线详情按钮只填入指令，等待用户补充玩家名；解绑玩家按钮同样不会自动发送，避免误触解绑。
- 关闭功能指令前缀后，功能按钮自动改用顶级指令，根按钮仍使用配置的 `commandPrefix`。

`qqMarkdownEnabled` 只控制查询结果是否自动使用 QQ Markdown 和公网图片，`qqKeyboardEnabled` 则控制“查在线”附带键盘及“按钮菜单”指令。两项可以独立开关。

## 调试日志

`verboseConsoleLog` 默认为 `false`。开启后会额外输出 API 请求与响应摘要、Typst 与字体诊断、QQ 图片缓存路径、图片大小、Markdown 发送参数，以及最终拼接出的完整临时图片 HTTP(S) URL。API 日志会隐藏 URL 中的 token，但临时图片 URL 在缓存有效期内可直接访问，因此不要将完整生产日志公开粘贴到聊天、Issue 或其他公共位置。
