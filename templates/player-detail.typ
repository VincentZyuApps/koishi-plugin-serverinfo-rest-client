#import "common.typ": *
#set page(width: 450pt, height: auto, margin: 14pt, fill: page-fill)
#set text(font: text-fonts, size: 11pt, fill: theme-color("text"), lang: "zh")

#let detail-panel(title, rows) = block(
  fill: theme-color("panel_fill"),
  stroke: 1pt + theme-color("panel_stroke"),
  radius: 4pt,
  inset: 12pt,
  width: 100%,
)[
  #text(weight: "bold", fill: theme-color("section_title"))[#title]
  #v(6pt)
  #table(
    columns: (auto, 1fr),
    stroke: none,
    row-gutter: 6pt,
    ..rows.map(row => (
      [#row.label],
      [#text(size: 9.5pt)[#row.value]],
    )).flatten(),
  )
]

#block(fill: theme-color("header_fill"), stroke: 2pt + theme-color("header_stroke"), radius: 6pt, inset: 10pt, width: 100%)[
  #align(center)[#text(size: 16pt, weight: "bold", fill: theme-color("header_text"))[#payload.label 👤 玩家 · #payload.name]]
]
#v(8pt)

#for (index, section) in payload.sections.enumerate() {
  detail-panel(section.title, section.rows)
  if index + 1 < payload.sections.len() { v(6pt) }
}
